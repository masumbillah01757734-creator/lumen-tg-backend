...
async function handleWebhook(req, res) {
  ...
  try {
    ...
    // Keep the profile picture in sync so the dashboard can show it next to their messages.
    try {
      const photoFileId = await telegramService.getUserProfilePhotoFileId(chat_id);
      user.profile_photo_file_id = photoFileId || null;
    } catch (err) {
      console.error("[Telegram] Failed to fetch profile photo:", err.message);
    }
